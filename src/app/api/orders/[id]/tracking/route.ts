...
export async function GET(
    ...
    try {
        ...
        const { data: order, error: orderError } = await supabase
            .from('orders')
            .select('id, order_number, user_id, status, tracking_number, carrier, shipped_at, delivered_at, created_at')
            .eq('id', params.id)
            .single()
        ...
        return NextResponse.json({
            data: {
                ...
                timeline,
                // In production, integrate with carrier API for real-time tracking
